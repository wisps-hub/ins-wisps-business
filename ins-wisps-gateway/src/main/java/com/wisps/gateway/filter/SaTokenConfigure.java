    ...
    public SaReactorFilter getSaReactorFilter() {
        return new SaReactorFilter()
                ...
                .setAuth(obj -> {
                    ...
                    SaRouter.match("/trade/**", r -> StpUtil.checkPermission(UserPerm.AUTH.name()));

                    SaRouter.match("/user/**", r -> StpUtil.checkPermissionOr(UserPerm.BASIC.name(), UserPerm.FROZEN.name()));
                    SaRouter.match("/order/**", r -> StpUtil.checkPermissionOr(UserPerm.BASIC.name(),UserPerm.FROZEN.name()));
                })
    ...
    private SaResult getSaResult(Throwable throwable) {
        switch (throwable) {
            ...
            case NotPermissionException notPermissionException:
                if (UserPerm.AUTH.name().equals(notPermissionException.getPermission())) {
                    log.error("请先完成实名认证！");
